...
static int __init gen3_pci_init(void)
{
	...
	switch (soc_device_id) {
		...
		raw_pci_ops = &ce_soc_pci_conf;
			...
			break;
		case PCI_DEVICE_ID_INTEL_CE2600_SOC:
#define CE2600_DOCSIS_MBAR_BASE 0xC8000000
#define CE2600_DOCSIS_MBAR_LENGTH 0x20000
			p = (uint32_t *)ioremap(CE2600_DOCSIS_MBAR_BASE, CE2600_DOCSIS_MBAR_LENGTH);
			if (p != NULL) {
				board = be32_to_cpu(readl(p + 0x1FF0/4)) - 1; // BOARD_TYPE offset
